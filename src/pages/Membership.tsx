  ...
  const handleUpgrade = async (tier: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Update local user state to reflect the new tier immediately
        setCurrentUser({
          ...currentUser,
          tier: tier as 'free' | 'premium' | 'professional' | 'elite',
        });
        ...
        // Show success toast
        addToast(`Successfully upgraded to ${tier.charAt(0).toUpperCase() + tier.slice(1)}!`, 'success');

        // Refetch user data from database to ensure everything is in sync
        setTimeout(async () => {
          try {
    ...
    <div className="min-h-screen pt-20 pb-12 px-4 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-playfair font-bold text-white mb-4">
            Creator <span className="gradient-text">Membership</span>
          </h1>
          <p className="text-gray-300 text-lg">Unlock premium features and grow your creator presence</p>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col justify-between">
              ...
              {nextTier && (
                <button
                  onClick={() => {
                    setSelectedUpgradeTier(nextTier);
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {isCurrentTier ? (
                      ...
                      <button
                        onClick={() => {
                          setSelectedUpgradeTier(tierKey);
                          setShowUpgradeModal(true);
                        }}
      ...
      {showUpgradeModal && selectedUpgradeTier && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => {
          ...
          <div className="glass-effect rounded-2xl p-8 border border-white/10 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-2xl font-bold text-white mb-4">Confirm Upgrade</h3>
            <p className="text-gray-300 mb-6">
              Upgrade your account to <span className="font-semibold capitalize text-rose-400">{selectedUpgradeTier}</span> tier to unlock premium features and benefits.
            </p>

            <div className="bg-white/5 rounded-lg p-4 mb-6 border border-white/10">
              <div className="text-gray-400 text-sm mb-1">New Tier</div>
              <div className="text-xl font-bold text-white capitalize">{selectedUpgradeTier}</div>
              <div className="text-gray-400 text-sm mt-2">{tierBenefits[selectedUpgradeTier].price} {tierBenefits[selectedUpgradeTier].billing}</div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                disabled={upgrading === selectedUpgradeTier}
                className="flex-1 py-3 bg-gradient-to-r from-rose-500 to-purple-600 text-white font-semibold rounded-lg hover:shadow-lg transition-all disabled:opacity-50"
              >
                {upgrading === selectedUpgradeTier ? 'Upgrading...' : 'Confirm Upgrade'}
              </button>
            </div>
          </div>
        </div>
